package com.lknhac.nlawyer.rate;
...
import com.lknhac.nlawyer.MainActivity;
import com.lknhac.nlawyer.rate.RateMeMaybeFragment.RMMFragInterface;
...
public class RateMeMaybe implements RMMFragInterface {
	...
	public RateMeMaybe(MainActivity mainActivity) {
		mActivity = mainActivity;
		mPreferences = mActivity.getSharedPreferences(Const.NAME, 0);
	}
	 ...
	 */
	public static void resetData(MainActivity mainActivity) {
		mainActivity.getSharedPreferences(PREF.NAME, 0).edit().clear().commit();
	...
	private void showDialog() {
		if (mActivity.getFragmentManager().findFragmentByTag(
				"rmmFragment") != null) {
...
//		frag.show(mActivity.getSupportFragmentManager(), "rmmFragment");
		frag.show(mActivity.getFragmentManager(), "rmmFragment");
	...
	public void _handlePositiveChoice() {
		Editor editor = mPreferences.edit();
		editor.putBoolean(Const.DONT_SHOW_AGAIN, true);
		editor.commit();
